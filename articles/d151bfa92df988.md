...
create アクションで新しい単語が作成されたときに、Action Cable を使用して 'chain_word' チャネルにブロードキャストを送信しています。

ChainWord レコードが正常に保存された場合、ActionCable.server.broadcast メソッドを使用して、'chain_word' チャネルに対してブロードキャストを行います。
...
レポート周り
Link-AG、Call-ASP、Ad Analytics のレポートを画面をリロードしなくても適宜更新されるようにする
チャットボット
社内チャットボットや、ゆくゆく Link-AG に導入した場合、AI からレスポンスが返ってきたら、それを反映させる
