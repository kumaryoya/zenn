...
この Rails アプリケーションは、User と Post は 1 対多の関係で、基本的な CRUD 機能を備えた、一般的なアプリケーションとなっています。
また、ログイン機能は [devise](https://github.com/heartcombo/devise) を使用して実装しており、投稿一覧画面にはログインユーザーが投稿した記事のみが表示されるようになっています。
...
- そのため、記事内では、あえて隠さずに実装手順を公開しています。
- 実際に実装する際は、認証情報やフォルダ、スプレッドシートを適切に管理してください。
:::

### 1. User に スプレッドシート ID を保存するカラムを追加

User 毎に異なるスプレッドシートにデータを書き込むため、User モデルに スプレッドシート ID を保存するカラムを追加する必要があります。

```ruby:ターミナル
docker compose exec app rails g migration AddGoogleSpreadsheetIdToUsers google_spreadsheet_id:string
```

```ruby:db/migrate/20250308134258_add_google_spreadsheet_id_to_users.rb
class AddGoogleSpreadsheetIdToUsers < ActiveRecord::Migration[7.0]
  def change
    add_column :users, :google_spreadsheet_id, :string
  end
end
```
...
今回の実装では、以下の 3 つの認証情報を Google Cloud から取得する必要があります。
...
JSON ファイルがダウンロードされれば、認証情報の取得は完了です。

![](https://storage.googleapis.com/zenn-user-upload/5c1cdffaabd2-20250309.png =1000x)

### 3. Google API の有効化

- 今回の実装では、以下の 2 つの API を有効化する必要があります。
...
今回の実装では、スプレッドシートを保存するためのフォルダを作成する必要があります。
...
*作成されたフォルダにアクセスし、URL からフォルダ ID を取得*

### 5. 環境変数の設定

今回の実装では、環境変数の設定に [dotenv](https://github.com/bkeepers/dotenv) を使用します。

`dotenv` は、`.env` ファイルに環境変数を記述し、アプリケーション内で使用できるようにする gem です。

本記事では、`dotenv` の導入手順については割愛します。
...
### 7. app/services/spreadsheet_service.rb の作成
...
### 8. app/controllers/posts_controller.rb の修正
...
+   SpreadsheetService.new.create_spreadsheet(current_user) if current_user.spreadsheet_id.blank?
+   SpreadsheetService.new.export_posts(current_user)
...
### 10. app/views/posts/index.html.slim の修正
...
今回は、Google API を使ってスプレッドシートにデータを書き込む方法について解説しました。

データを書き込む処理をバックグラウンドで行うようにするや、定期実行されるようにするなど、機能をアップグレードする余地はまだまだあります。

スプレッドシートに関しても、同じシートにデーカルデータを書き込むのではなく、日別や月別でシートを分けてデータを書き込むなど、様々な使い方が考えられます。

Google API を使ったスプレッドシートの活用方法は、様々な場面で活用できるので、ぜひ活用してみてください。

最後までご覧いただきありがとうございました。

## 参考文献

https://kossy-web-engineer.hatenablog.com/entry/2021/04/10/google_api_client
