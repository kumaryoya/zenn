...
今回はその中でも特に効果が大きい以下の 3 つを整備しました。
...
これらを CLAUDE.md に書いておくことで、問題①の「環境を理解していないので的外れなコマンドを実行する」や問題③の「毎回ゼロから把握するので時間がかかる」がほぼ解消されました。
...
`rubocop` を整備しておけば違反時の対処も自動で進みますし、`update-claude-md` を整備しておけば「CLAUDE.md 更新して」の一言で常に最新の状態を保てるようになります。

### agents/

agents/ には、特化型のサブエージェントを定義します。

例えば、`performance-analyzer` という「パフォーマンス分析専門のエージェント」を作成すると、N+1 クエリの調査やパフォーマンスチューニングを依頼するときに、独立したコンテキストで作業を任せられます。
