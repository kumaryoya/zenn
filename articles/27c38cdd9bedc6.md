---
title: "【Claude Code】「育成」の 3 ファイルで、的外れな提案が 1 日 5〜10 回 → 0 回に"
emoji: "🌱"
type: "tech"
...
「Claude Code、入れてみたけど思ったほど効率が上がらない」——そう感じている人は多いのではないでしょうか。

実は、Claude Code は**入れただけでは真価を発揮できない**ツールです。

新入社員のオンボーディングと同じで、Claude Code にもプロジェクトの前提知識・チームのルール・タスクの進め方を渡す「育成」が必要です。

私自身、最初は VS Code の拡張機能を入れただけの状態で使っていて、「思っていたほど効率が上がらないな」と感じていました。

そこで Claude Code の設定まわりを調べ、いくつかのファイル・ディレクトリを整備していった結果、**1 日 5〜10 回受けていた的外れな提案が、ほぼ 0 回になりました**。
...
本記事では、それぞれの役割と、実際に整備した中身を順に共有します。
...
## あえて整備しなかった `commands/` と `rules/`

調べていく中で「これとこれって何が違うの？」と気になったポイントがあったので、整理しておきます。

### `rules/` は使わず、`CLAUDE.md` だけにした
...
- [CLAUDE.md / rules/（Memory）](https://code.claude.com/docs/ja/memory)
- [skills/（Skills）](https://code.claude.com/docs/ja/skills)
- [agents/（Sub-agents）](https://code.claude.com/docs/ja/sub-agents)
