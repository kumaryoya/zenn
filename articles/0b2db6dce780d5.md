...
今回は、「Railsエンジニアが知っておきたい配列とハッシュの違いと使い分け」ということで、**Ruby**の基本であり、**Rails**開発でも頻繁に登場する**配列（Array）**と**ハッシュ（Hash）**について、違いや使い分けをまとめてみました。

「なんとなく使い分けている」という方も多いかと思いますが、改めて整理してみると、コードの可読性やパフォーマンスに大きく影響することが分かりました。
...
**配列**は、複数の値を**順番に**並べて管理するデータ構造です。
各要素には**インデックス（0始まり）**でアクセスします。
...
fruits.select { |fruit| fruit.length > 2 }
...
**ハッシュ**は、**キーと値のペア**でデータを管理するデータ構造です。
各要素には**キー**でアクセスします。

```ruby
user = { name: "おおくま", age: 25, role: "エンジニア" }
...
# キー・値の取得
user.keys    #=> [:name, :age, :role, :email]
user.values  #=> ["おおくま", 26, "エンジニア", "example@test.com"]
...
```ruby
# 配列 → ハッシュ
pairs = [[:name, "おおくま"], [:age, 25]]
pairs.to_h  #=> { name: "おおくま", age: 25 }

# ハッシュ → 配列
user = { name: "おおくま", age: 25 }
user.to_a  #=> [[:name, "おおくま"], [:age, 25]]
