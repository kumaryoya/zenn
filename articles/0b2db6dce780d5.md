...
特定の値が含まれているかを頻繁にチェックする場合、配列よりハッシュを使った方がパフォーマンスが向上します。
...
# 推奨：ハッシュで検索（速い）
blocked_user_ids = BlockedUser.pluck(:user_id).index_with(true)
# => { 1 => true, 5 => true, 10 => true, 15 => true, ... }

users.each do |user|
  next if blocked_user_ids[user.id]  # O(1) × ユーザー数
end
```
...
:::message
index_by は Rails が提供するメソッドで、配列からハッシュを簡単に作成できます。
...
- Rails では `index_by` や `index_with` を活用することで、効率的なコードが書ける

適切に配列とハッシュを使い分け、可読性やパフォーマンスを向上させていきましょう。
