...
ここで、`Post`一覧画面を作成する際に、わざと**N+1問題**を発生させてみます。
...
このように、コントローラーでは`Post`の情報のみを取得し、ビューで個別に`User`の情報を取得しているため、**N+1問題**が発生しています。
...
画面を表示するために、`User`テーブルに対して、`Post`の数だけクエリが発行されていることがわかります。

`Post`の数が増えるほど、発行されるクエリの数も増えてしまうため、パフォーマンスが低下してしまいます。

### 解決方法

今回の例で言えば、`Post`の情報を取得する際に、`User`の情報も一緒に取得するように修正することで、**N+1問題**を解消することができます。
...
発行されたクエリの数が減ったことがわかります。

コードを少し修正するだけで、**N+1問題**を解消することができます。
...
**bullet**を導入することで、**N+1問題**が発生している箇所を教えてくれるため、**N+1問題**を解消することができます。
...
- `Bullet.enable`：**Bullet**を有効化(これを`true`にしないと機能しない)
- `Bullet.alert`：**N+1問題**を発見したらJavaScriptのアラート(ポップアップ)で通知
- `Bullet.bullet_logger`：`log/bullet.log`に**N+1問題**の詳細を記録
- `Bullet.console`：ブラウザの開発者ツールのコンソールに**N+1問題**を表示
- `Bullet.rails_logger`：Railsのログ(`log/development.log`)にも記録
- `Bullet.add_footer`：ページのフッターに**N+1問題**の情報を表示

これで、**bullet**を導入する準備が整いました。

## 試してみる

先ほど修正した`Post`一覧画面で、**N+1問題**を再度発生させてみます。
...
`Post`一覧画面を表示すると、以下のようなアラートが表示されます。
...
`log/bullet.log`にも**N+1問題**の詳細が記録されていることがわかります。

![](https://storage.googleapis.com/zenn-user-upload/9fe2b00afe98-20250211.png =500x)

ブラウザの開発者ツールのコンソールにも**N+1問題**が表示されています。
...
Railsのログ(`log/development.log`)にも**N+1問題**の詳細が記録されていることがわかります。

![](https://storage.googleapis.com/zenn-user-upload/be9b68d6ed27-20250211.png =500x)

ページのフッターにも**N+1問題**の情報が表示されています。

これで、**bullet**を使って、**N+1問題**を可視化することができました。

もし、開発中に**N+1問題**が発生してしまっても、**bullet**を使うことで、すぐに気づくことができます。
