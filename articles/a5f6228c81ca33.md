...
現状、IDが1の**Article**レコードは暗号化されていないデータです。
試しに、IDが1の**Article**レコードとIDが2の**Article**レコードをそれぞれ取得してみます。
...
このように、暗号化されているデータを取得することはできますが、暗号化されていないデータを取得しようとすると、**Active Record**が暗号化されていないデータを復号しようとするため、エラーが発生します。
...
現状、データベースには、以下のように暗号化データと非暗号化データが混在しています。
