...
今回は、日本からのアクセスのみを許可し、それ以外は全てブロックする設定を行います。
...
次に、以下の項目を順に選択・入力していきます。

- **Resource type**：AWS WAF を設定したいリソースに応じて、Global resources または Regional resources を選択します。

- **Region**：Resource type で Regional resources を選択した場合、対象のリージョンを選択します。
...
- **Associated AWS resources**：AWS WAF を設定するリソースを選択します。

- **Resource level DDoS protection**：Associated AWS resources で Application Load Balancer を選択した場合、DDoS 保護のレベルを選択します。
...
- **Rule type**：ルールの種類を選択します。今回は「Rule builder」を選択します。
...
AWS WAF を使用して、海外からのアクセスを制限する方法について解説しました。

海外からのアクセスを制限することで、セキュリティ面だけでなく、パフォーマンス面においても効果があると思います。
